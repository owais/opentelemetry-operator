...
func testAgentConfig() config.StandaloneAgentConfig {
	return config.StandaloneAgentConfig{
		...
		Config: map[string]config.StandaloneConfigEntry{
			"collector": {
				Kind: config.StandaloneConfigEntryKindConfigMap,
				Name: "collector-config",
				Key:  "collector.yaml",
			},
		},
...
func TestScopedApplierApplyRejectsInvalidCollectorConfig(t *testing.T) {
	c := newTestClient(getFakeK8sClient(t, testConfigMap(), testDeployment()))

	invalidConfig := `receivers:
  otlp:
    protocols:
      grpc:
...
`
	...
	assert.Contains(t, err.Error(), "the OpenTelemetry config is incorrect")
}
